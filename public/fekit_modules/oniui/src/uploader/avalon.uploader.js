var avalon = require("../browser/avalon.browser"),
sourceHTML = "<input type=\"file\" name=\"file[]\" multiple style=\"filter: alpha(opacity=0); height: 0; width: 0;\" />\nMS_OPTION_COM\n<div style=\"position: absolute; width: 100%; height: 100%; left: 0; top: 0; opacity: 0; filter: alpha(opacity=0);\">\n\t<div id=\"altContent\">\n\t\t<h1>ExifUpload</h1>\n\t\t<p><a href=\"http://www.adobe.com/go/getflashplayer\">Get Adobe Flash player</a></p>\n\t</div>\n</div>";
...
function () {
    var widget = avalon.ui.uploader = function (element, data, vmodels) {
            var uploaderId = data.uploaderId, swfId = uploaderId + 'Swf', swf,
                // {Element} <object>
                fileList = [],
                // 存放file对象
                browseButton,
                ...
                isSafe = true,
                // 配置项安全检测标志
                ieVersion = avalon.browser.ie, hasLoadFlash = false;
            var vmodel = avalon.define(data.uploaderId, function (vm) {
                    ...
                    vm.deleteFile = function (index) {
                        avalon.post(vmodel.action, { id: vmodel.files[index].id }, function (data) {
                            if (data.errcode == 0) {
                                // 成功
                    ...
                    vm.$jsHandler = function (obj) {
                        ...
                        case 'singleError':
                            ...
                            break;
                        case 'uploaded':
                            var items = obj.data.sucAry, imgs = [];
		 ...
		 */
            ...
            function loadInput() {
                ...
                input.onchange = function (e) {
					 ...
					 */
                        this.select();
                        this.blur();
                        vmodel.files.push({ src: document.selection.createRange().text });
                    }
                    // 清空input，保证下次change事件的触发
                    this.value = '';
                    // ie10-下无法清空，借用form的reset方法
                    if (this.value) {
                        var _tempForm = document.createElement('form'), _nextElement = this.nextSibling, _parentElement = this.parentNode;
                        // 取出并清空该input
            ...
            function loadFlash() {
                ...
                avalon.swfobject.embedSWF('swfobject/multiPicUpload.swf', 'altContent', '100%', '100%', '10.0.0', 'expressInstall.swf', flashvars, params, attributes);
                hasLoadFlash = true;
            }
