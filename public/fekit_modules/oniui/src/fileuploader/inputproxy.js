...
function () {
    ...
    proxyContructor.prototype.onH5FileFieldChanged = function (event) {
        ...
        var html = target.outerHTML, pNode = target.parentNode, fileInputWrapper = document.createElement('div');
        if (!pNode)
            return;
    ...
    proxyContructor.prototype.getImagePreview = function (fileInfo, previewWidth, previewHeight, callback) {
        ...
        var promise = new Promise(function (resolve, reject) {
                ...
                fileReader.onload = function () {
                    var img = new Image();
                    img.onload = function () {
                        var canvasEl = document.createElement('canvas'), ctx = canvasEl.getContext('2d'), imgWidth = img.width, imgHeight = img.height, imageAspectRatio = imgWidth / imgHeight,
                            // 输入图片的宽高比
                            canvasAspectRatio = previewWidth / previewHeight,
                            // 预览的宽高比
                            drawX = drawY = 0, drawHeight = previewHeight,
                            // 绘制入Canvas中的预览图高度
