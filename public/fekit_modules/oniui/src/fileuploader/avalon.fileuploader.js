...
function () {
    ...
    var widget = avalon.ui[widgetName] = function (element, data, vmodels) {
            ...
            var vmodel = avalon.define(data[widgetName + 'Id'], function (vm) {
                    ...
                    var supportMultiple = document.createElement('input').multiple != undefined, supportFile = window.File != undefined, supportCanvas = document.createElement('canvas').getContext && document.createElement('canvas').getContext('2d'), isIE = avalon.browser.name.toLowerCase() == 'ie', version = avalon.browser.version;
                 ...
                 */
                    vm.registInput = function (opts, target, isH5) {
                        ...
                        opts.$fileInputProxy.addEventListener('previewGenerated', function (fileLocalToken, preview) {
                            ...
                            if (!previewVm || previewVm.preview == preview)
                                return;
                 ...
                 */
                    vm.getInputAcceptTypes = function (_isFalsh) {
                        ...
                        if (_isFalsh) {
                            ...
                            var getMIME4SpecialType = function (category) {
                                ...
                                allFilters.push({
                                    description: categoryMime.replace('/', ''),
                                    types: filters.join(';')
                                });
                            };
                            // 处理特殊文件类型
                    ...
                    vm.$init = function () {
                        ...
                        element.innerHTML = template.replace(/##VM_ID##/gi, vm.$id);
                        // 将vmid附加如flash的url中
    ...
    widget.defaults = {
        ...
        //@config {Number} 单个文件的大小限制
        filePoolSize: 1024 * 1024 * 200,
        //@config {Number} 未上传文件的总大小限制。
        chunkSize: 1024 * 1024,
        //@config {Number} 分块上传时的分块大小。
        chunked: false,
        //@config {Boolean} 是否开启分块上传。
        __inputRegisted: false,
        addButtonText: 'Add Files',
        //@config {String} 添加文件按钮的文本
        uploadButtonText: 'Upload',
        //@config {String} 上传按钮的文本
        acceptFileTypes: '*.*',
        //@config {String} 可接收的文件类型
        previewWidth: 100,
        //@config {Number} 预览图的宽度
        previewHeight: 85,
        //@config {Number} 预览图的高度
        enablePreviewGenerating: true,
        //@config {Boolean} 是否开启预览图的生成功能
        showPreview: true,
        //@config {Boolean} 是否显示预览图
        showProgress: true,
        //@config {Boolean} 是否显示进度条
        multipleFileAllowed: true,
        enableRemoteKeyGen: false,
        //@config {Boolean} 分块上传时，是否预先和服务器握手，获取一个文件Id
        enableMd5Validation: false,
        //@config {Boolean} 是否开启文件分块MD5验证，开启后每个文件请求内都会附加上文件数据的Md5码
        /*
        ...
        uploadFile: function (opts, index) {
            var fileLocalToken = undefined, inVarType = typeof index;
            if (inVarType == 'number') {
        ...
        testFileSize: function (opts, fileObj) {
            var fileSizeLimitation = opts.maxFileSize, poolSizeLimitation = opts.filePoolSize, size = fileObj.size;
