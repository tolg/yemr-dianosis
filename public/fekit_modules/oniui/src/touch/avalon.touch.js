...
function () {
    void function () {
        ...
        var self = avalon.bindingHandlers.on = function (data, vmodels) {
                ...
                if (typeof self[eventType + 'Hook'] === 'function') {
                    //添加钩子
        ...
        self['clickHook'] = function (data) {
            var tapping = false, element = data.element, fastclick = avalon.fastclick, doubleIndex = 0,
                //用于决定何时重置doubleStartTime
                doubleStartTime,
                //双击开始时间,
                startTime,
                // 单击开始时间
        ...
        document.addEventListener('click', function (e) {
            ...
            if (target.href && target.href.match(/#(\w+)/)) {
                var id = RegExp.$1;
                if (id) {
                    var el = document.getElementById(id)    //这里做锚点的滚动处理,或做在scroll插件中
;
                }
            }
        }, true);
        ...
        avalon.fastclick = {
            ...
            clickDuration: 750,
            //小于750ms是点击，长于它是长按或拖动
            dragDistance: 14,
            //最大移动的距离
            preventTime: 2500,
            //2500ms还原ghostPrevent
        ...
        if (touchSupported) {
            (function (DOC) {
                var touchProxy = {}, touchTimeout, tapTimeout, swipeTimeout, holdTimeout, now, firstTouch, _isPointerType, delta, deltaX = 0, deltaY = 0, touchNames = [], rotateThreshold = 15,
                    // 判断方向的角度
                    pinchThreshold = 10;
                // 判定 pinch 的位移偏移量
                ...
                function getDirection(offsetX, offsetY) {
                    var ret = [], absX = Math.abs(offsetX), absY = Math.abs(offsetY), proportion = Math.tan(rotateThreshold / 180 * Math.PI), transverse = absX > absY;
                    if (absX > 0 || absY > 0) {
                ...
                function isPrimaryTouch(event) {
                    //是否纯净的触摸事件，非mousemove等模拟的事件，也不是手势事件
                ...
                function compute(e, len) {
                    var touches = e.touches, len = touches.length, firstTouch = touches[0], otherTouch = touches[1], disX = firstTouch.clientX - otherTouch.clientX, disY = firstTouch.clientY - otherTouch.clientY;
                    return {
                ...
                DOC.addEventListener(touchNames[1], function (e) {
                    ...
                    if (e.touches.length > 1 && touchProxy.status && touchProxy.status.match(/other|pinch/g)) {
                        ...
                        if (touchProxy.pinch) {
                            var direction = changes > 0 ? 'in' : 'out', name = touchProxy.status !== 'pinch' ? [
                                    'pinch',
                                    'pinch' + direction
                                ] : [changes > 0 ? 'splay' : 'squeeze'];
                ...
                DOC.addEventListener(touchNames[2], function (e) {
                    ...
                    if (status === 'other' || status === 'pinch') {
                        touchProxy = {}    // swipe
;
                    } else if (touchProxy.x2 && Math.abs(touchProxy.x1 - touchProxy.x2) > 30 || touchProxy.y2 && Math.abs(touchProxy.y1 - touchProxy.y2) > 30) {
                        ...
                        swipeTimeout = setTimeout(function () {
                            ...
                            touchProxy.fire('swipe' + direction, { direction: direction });
                            touchProxy = {};
                        }, 0)    // normal tap 
;
            ...
            }(document)    //http://quojs.tapquo.com/ http://code.baidu.com/
